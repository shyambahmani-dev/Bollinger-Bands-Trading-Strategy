...
def getDMA(data1, intervals = [10, 25, 50, 100, 200]):
    ...
    try:
        ...
        for it in intervals:
            ...
            data1DMA["%d" %(it)] = data1["Close"].rolling(it).mean()
            data1DMA["%d" %(it)] = data1DMA["%d" %(it)].dropna()
...
def getEMA(data1, intervals = [10, 25, 50, 100, 200]):
    ...
    try:
        ...
        for it in intervals:
            ...
            data1EMA["%d" %(it)] = data1["Close"].ewm( span = it ).mean()
            data1EMA["%d" %(it)] = data1EMA["%d" %(it)].dropna()
...
def getRSI(data1):
    ...
    try:
        ...
        data1RSI = pd.DataFrame() # columns = ['Change', 'Gain', 'Loss', 'Avg Gain', 'Avg Loss', 'RS', 'RSI']) # columns = data1.columns , index = data1.index )

        #data1RSI = data1RSI.astype(dtype={'Change':float, 'Gain':float, 'Loss':float, 'Avg Gain':float, 'Avg Loss':float, 'RS':float, 'RSI':float})
...
def getVolatility(data1, interval = 10):

    try:

        data1Volatility = pd.DataFrame()
        data1Volatility = data1.rolling(window = interval).std()

        return data1Volatility
